...
AuctioneerAddress=$(cat data/blast/deployment.json | jq -r ".contracts.Auctioneer")

# Get Deployment first block
FirstBlock=$(cat data/blast/deployment.json | jq -r ".firstBlock")
# FirstBlock=50
...
# Update chain
echo $(cat subgraph.yaml | yq -y '.dataSources[0].network = "blast-mainnet"') > subgraph.yaml
